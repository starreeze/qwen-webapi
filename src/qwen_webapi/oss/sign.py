...
from .types import HttpRequest, Signer, SigningContext
...
class SignerV4(Signer):
    ...
    def sign(self, signing_ctx: SigningContext) -> None:
        if signing_ctx.credentials is None or not signing_ctx.credentials.has_keys():
            raise ValueError("SigningContext.credentials is required")

        if signing_ctx.request is None:
            raise ValueError("SigningContext.request is required")
    ...
    def _auth_header(self, signing_ctx: SigningContext) -> None:
        ...
        if cred.security_token is not None:
            request.headers.update({"x-oss-security-token": cred.security_token})
        ...
        scope = self._build_scope(date=date_now_iso8601, region=region, product=product)

        # additional headers
        additional_headers = self._common_additional_headers(request, signing_ctx.additional_headers)
        ...
        # string to sign
        string_to_sign = self._calc_string_to_sign(datetime_now_iso8601, scope, canonical_request)
    ...
    def _auth_query(self, signing_ctx: SigningContext) -> None:
        ...
        if cred.security_token is not None:
            encoded_pairs.update({"x-oss-security-token": quote(cred.security_token, safe="")})

        if len(additional_headers) > 0:
            encoded_pairs.update({"x-oss-additional-headers": quote(";".join(additional_headers), safe="")})
    ...
    def _common_additional_headers(
        self, request: HttpRequest, additional_headers: Optional[Set[str]] = None
    ) -> Set[str]:
        keys = set()
    ...
    def _calc_canonical_request(
        self, signing_ctx: SigningContext, additional_headers: Optional[Set[str]] = None
    ) -> str:
        """
        Canonical Request
            HTTP Verb + "\n" +
        ...
        # hash payload
        hash_payload = request.headers.get("x-oss-content-sha256", "UNSIGNED-PAYLOAD")
    ...
    def _calc_signature(
        self, access_key_secrect: str, date: str, region: str, product: str, string_to_sign: str
    ) -> str:
        key_secret = ("aliyun_v4" + access_key_secrect).encode("utf-8")
        signing_date = hmac.new(key_secret, date.encode("utf-8"), sha256).digest()
        signing_region = hmac.new(signing_date, region.encode("utf-8"), sha256).digest()
        signing_product = hmac.new(signing_region, product.encode("utf-8"), sha256).digest()
        signing_key = hmac.new(signing_product, "aliyun_v4_request".encode("utf-8"), sha256).digest()
